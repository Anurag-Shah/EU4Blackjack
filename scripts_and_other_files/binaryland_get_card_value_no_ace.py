...
if __name__ == "__main__":
	...
	for i in range(1,5):
		for j in range(1, 19):
			cond = 'check_variable = { which = blackjack_player_' + str(i) + '_hand_' + str(j) + ' value = %s }'
			body = 'set_variable = { which = export_variable value = %s }'
			form = 'if = {\n\t\tlimit = {\n\t\t\t%s\n\t\t}\n\t\t%s\n\t}\n\telse = {\n\t\t%s\n\t}' % (cond, '%s', '%s')
			stro += 'get_card_value_no_ace_player_'
			stro += str(i)
			stro += '_hand_'
			stro += str(j)
			stro += ' = { #player #position\n\t'+btree([i for i in range(1, 53)], form, body)+'\n}\n\n'
