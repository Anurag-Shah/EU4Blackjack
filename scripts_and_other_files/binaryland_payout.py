...
if __name__ == "__main__":
	stro = ""
	for i in range(1,5):
		cond = 'check_variable = { which = bet_value_p' + str(i) + ' value = %s }'
		body = 'add_treasury = %s'
		body_alt = 'add_treasury = -%s'
		form = 'if = {\n\t\tlimit = {\n\t\t\t%s\n\t\t}\n\t\t%s\n\t}\n\telse = {\n\t\t%s\n\t}' % (cond, '%s', '%s')
		
		stro += 'blackjack_payout_w_p' + str(i) + ' = {\n\t'+btree([i for i in range(0, 15000)], form, body)+'\n\n}'
		stro += 'blackjack_payout_l_p' + str(i) + ' = {\n\t'+btree([i for i in range(0, 15000)], form, body_alt)+'\n\n}'
