...
if __name__ == "__main__":
	stro = ''
	for i in range(1,5):
		cond = 'check_variable = { which = $in_var$ value = %s }'
		body = 'event_target:blackjack_player_1_tag = { set_variable = { which = $out_var$ which = blackjack_player_' + str(i) + '_hand_%s } }'
		form = 'if = {\n\t\tlimit = {\n\t\t\t%s\n\t\t}\n\t\t%s\n\t}\n\telse = {\n\t\t%s\n\t}' % (cond, '%s', '%s')
		stro += 'blackjack_obj_card_val_at_pos_player_' + str(i) + ' = { \n\t'+btree([i for i in range(1, 19)], form, body)+'\n}\n\n'
